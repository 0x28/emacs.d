...
* org
  #+begin_src emacs-lisp
    (defun my/org-tag ()
      "Toggle org-mode tags of the current heading with completion."
      (interactive)
      (let* ((current-tags (org-get-tags nil t))
             (new-tag (completing-read "toggle tag: " (org-get-buffer-tags))))
        (org-set-tags
         (if (member new-tag current-tags)
             (remove new-tag current-tags)
           (cons new-tag current-tags)))))
...
* ivy
  #+begin_src emacs-lisp
    ...
    (use-package counsel
      ...
      (counsel-async-filter-update-time 100000) ;; every 0.1 seconds
...
* CMake
  #+begin_src emacs-lisp
    (use-package cmake-mode
      ...
      :config
      (evil-define-key 'normal cmake-mode-map (kbd "SPC h h") #'cmake-help)
      :custom
      (cmake-tab-width 4))
  #+end_src
